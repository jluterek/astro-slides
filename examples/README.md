...
| [`morph-reel`](./morph-reel/) | The showcase: twelve dots that never leave the stage — they fly between formations (cluster → ring → rising line → arrow → constellation → wordmark), each keeping its identity via a `data-morph` id so the View Transitions API glides them to their new spots. Pure "magic move" motion, no dissolve. Present it in Chrome/Safari for the full effect. |
